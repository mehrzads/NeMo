...
from nemo.collections.asr.parts.submodules.zilant_modules import ZilantLayer
...
__all__ = ['ZilantEncoder']


class ZilantEncoder(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
...
class ZilantEncoderAdapter(ZilantEncoder, adapter_mixins.AdapterModuleMixin):
...
class ZilantMultiLayerFeatureExtractor(NeuralModule, Exportable, AccessMixin):
    """
...
"""
if adapter_mixins.get_registered_adapter(ZilantEncoder) is None:
    adapter_mixins.register_adapter(base_class=ZilantEncoder, adapter_class=ZilantEncoderAdapter)


@dataclass
class ZilantChangeConfig:
