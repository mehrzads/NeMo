...
from nemo.collections.asr.parts.submodules.conformer_modules import (ConformerLayer, MNSConformerLayer)
...
__all__ = ['MNSConformerEncoder']


class MNSConformerEncoder(NeuralModule, StreamingEncoder, Exportable, AccessMixin):
    """
    ...
    def __init__(
        ...
        if self_attention_model == "rel_pos":
            ...
            self.pos_enc = PositionalEncoding(
                d_model=d_model**2, dropout_rate=dropout_pre_encoder, max_len=pos_emb_max_len, xscale=self.xscale
            )
        else:
    ...
    def forward_internal(
        ...
        audio_signal = audio_signal.reshape(b, c, t, -1)
        ...
        for lth, (drop_prob, layer) in enumerate(zip(self.layer_drop_probs, self.layers2D)):
...
class MNSConformerEncoderAdapter(MNSConformerEncoder, adapter_mixins.AdapterModuleMixin):
...
"""
if adapter_mixins.get_registered_adapter(MNSConformerEncoder) is None:
    adapter_mixins.register_adapter(base_class=MNSConformerEncoder, adapter_class=MNSConformerEncoderAdapter)
